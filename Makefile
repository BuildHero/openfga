...
build-ios-simulator: ## Build the OpenFGA service for iOS Simulator
	gomobile bind -o ./outputs/MobileOpenFGA.xcframework -ldflags='-s -w' -target=iossimulator -tags=netgo ./pkg/mobile_openfga

.PHONY: build-ios
build-ios: ## Build the OpenFGA service for iOS Simulator
	gomobile bind -o ./outputs/MobileOpenFGA.xcframework -ldflags='-s -w' -target=ios -tags=netgo ./pkg/mobile_openfga

.PHONY: build-android
build-android: ## Build the OpenFGA service for Android
	CGO_CFLAGS= gomobile bind -o ./outputs/mobileopenfga.aar -ldflags='-s -w' -androidapi 26 -target=android -tags=netgo ./pkg/mobile_openfga
